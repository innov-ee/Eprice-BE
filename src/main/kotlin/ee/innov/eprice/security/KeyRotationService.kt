...
object KeyRotationService {
    // Fallback secret for local development/testing; MUST be overridden via MASTER_SECRET env var in production
    private val masterSecret: ByteArray by lazy {
        (System.getenv("MASTER_SECRET") ?: "dev-eprice-master-secret-change-in-production-32b")
     ...
     */
    fun getKeyInfo(now: Instant = Instant.now()): KeyRotationInfo {
     ...
     */
    fun isValid(key: String?, now: Instant = Instant.now()): Boolean {
